...
class Custom_Data(Dataset):
    def __init__(self, truth_dir: str, blured_dir: str, angle, val, test, transform=None) -> None:
        ...
        self.kernel_angle    = self.angle_loader(self.angle ,val, test)

    def angle_loader(self, json_file: str, val, test):
        f      = open(json_file)
        ang    = json.load(f)
        angles = {}
        if test:
            self.len  = int(len(ang["angle"][0]))

        else:
          if val :
            self.len  = int(len(ang["angle"][0])*0.8)
          else:
            self.len  = int(len(ang["angle"][0])*0.8)

        for i,item in enumerate(ang["angle"][0]):
            if test:
                angles[i] = ang["angle"][0][item]
            else:
                if val:
                    if i < self.len:
                        continue
    ...
    def data_loader(self,path, val=False, test=False):
        ...
        if test:
          ...
          if val :
            file  = sorted(paths)[int(len(paths)*0.8):]
          else:
            file  = sorted(paths)[:int(len(paths)*0.8)]
        data_list = []
...
def load_data(truth_d, deblured_d, angle, val=False, test=False):
    data_set      = Custom_Data(truth_d, deblured_d, angle, val = val, test=test
                          ) 
...
if __name__ == '__main__':
    ...
    test_angle            =  
    epochs                = 550
    training_data         = load_data(gt_dir, filter_dir,train_angle, val=False)
    val_data              = load_data(gt_dir, filter_dir,train_angle, val=True)
    test_data             = load_data(test_gt_dir, test_dir,test_angle, test=True) #test_loader(test_dir) 
    ...
    model                 = Convolution_4(3).to(device)
